...
### Three Ways to Start with DocFlow

#### 1. New Project (No Code Yet)
Copy DocFlow template into your project directory and run:
```
/new-project
```
The DocFlow agent will interview you about your vision, help you choose a stack, build an initial backlog, and create a project scaffolding task. Perfect for starting from scratch.

#### 2. Existing Project (Add DocFlow to Your Code)
Copy DocFlow template into your existing codebase and run:
```
/scan-project
```
The DocFlow agent will analyze your code, understand what it does, document existing features, and retrofit DocFlow. Great for inherited codebases or adding structure to existing projects.
...
DocFlow includes several commands (type `/` in Cursor to see them):

- **`/new-project`** - Set up a new project from scratch (planning first, code second)
- **`/scan-project`** - Analyze existing code and retrofit DocFlow documentation
- **`/start-session`** - Begin work session, check status, activate work
...
DocFlow uses a two-agent approach in Cursor:

### DocFlow Agent (Planning & Review)
- Strategic planning and spec management
- Creates and refines specs
- Reviews completed implementations
- Maintains DocFlow structure
- Runs: `/new-project`, `/scan-project`, `/start-session`, `/wrap-session`, `/capture`, `/review`
...
- Hands back to DocFlow for review

**Workflow:** DocFlow prepares specs → Implementation builds → DocFlow reviews → Repeat
